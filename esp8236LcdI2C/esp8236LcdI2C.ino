...
void loop() {
       ...
       delayMicroseconds(100);
        ...
        Path = path+"rate";
        Firebase.setDouble(firebaseData, Path.c_str(), flowMilliLitres);
        ...
        if(Firebase.getDouble(firebaseData, Path.c_str())){
        Serial.println("TYPE: " + firebaseData.dataType());}
        Serial.print(Firebase.getDouble(firebaseData, Path.c_str()));
        Firebase.setDouble(firebaseData, Path.c_str(), totalMilliLitres);
        Serial.println("");

        volbil = 10*bill;
        while(totalMilliLitres > volbil){
            lcd.clear();
